...
class RtpPacket:
        ...
        if self.extension and len(data) >= header_len + 4:
            ...
            if len(data) >= ext_data_end:
                if profile == 0xBEDE:
                    ...
                    # RFC5285 one-byte header extensions use profile 0xBEDE.
                    # Some senders also pack custom TLVs in the extension
                    # payload while still using 0xBEDE, so try both decoders.
                    self._parse_one_byte_extension(ext_data)
                    if not self.ma_name and not self.ma_player_id:
                        self._parse_tlv_extension(ext_data)
                elif profile == 0x1000:
                    # RFC5285 two-byte extension profile.
                    self._parse_tlv_extension(data[ext_data_start:ext_data_end])
    ...
    def _parse_one_byte_extension(self, ext_data: bytes) -> None:
        ...
        while idx < data_len:
            ...
            if ext_id == 1:
                self.ma_name = self._decode_ext_string(value)
            elif ext_id == 2:
                self.ma_player_id = self._decode_ext_string(value)
    ...
    @staticmethod
    def _decode_ext_string(raw: bytes) -> Optional[str]:
        """Decode a UTF-8 extension string and normalise wire padding."""
        if not raw:
            return None
        text = raw.decode("utf-8", errors="ignore")
        # Streams often pad extension values with NULs/spaces for alignment.
...
class _PlayerStream:
    ...
    def _handle_rtp(self, data: bytes) -> None:
        ...
        if pkt.ma_name and self._registry and self._source_ip:
            logger.debug(
                f"Player '{self.name}': MA RTP metadata "
                f"name='{pkt.ma_name}', player_id='{pkt.ma_player_id}' "
                f"from {self._source_ip} SSRC=0x{pkt.ssrc:08X}"
            )
